...
function DirectoryPage({ type }: { type: "customers" | "suppliers" }) {
  ...
  const [rows, setRows] = useState(isCustomers ? customerRows : supplierRows);
  ...
  function addDirectoryRecord() {
    setRows((current) => [
      isCustomers
        ? ["New Customer Ltd", "Pending TIN", "billing@newcustomer.ng\n+234 800 000 1000", "0", "Review"]
        : ["New Supplier Ltd", "Pending TIN", "General", "₦0", "Review"],
      ...current,
    ]);
  }
  ...
  return (
    <>
      ...
      <Card className="mb-6 p-4"><div className="flex min-w-0 flex-wrap gap-3">{(isCustomers ? ["All Customers", "Missing TIN", "Active", "Recent"] : ["All Suppliers", "Missing TIN", "Verified", "Recent"]).map((tab) => <button key={tab} type="button" onClick={() => { setActiveTab(tab); notifyDashboard(`${tab} filter applied`); }} className={`rounded-full px-4 py-2 text-sm font-bold ${activeTab === tab ? "bg-[#DADEFD] text-[#0001B1]" : "text-[#454557]"}`}>{tab}</button>)}<Button variant="secondary" className="w-full sm:ml-auto sm:w-auto" onClick={() => notifyDashboard("Advanced filters opened")}><Filter className="h-4 w-4" /> Advanced Filter</Button><Button variant="secondary" className="w-full sm:w-auto" onClick={() => notifyDashboard(`${isCustomers ? "Customer" : "Supplier"} CSV exported`)}><Download className="h-4 w-4" /> Export CSV</Button></div></Card>
      <DataTable title={isCustomers ? "Customer Records" : "Supplier Records"} columns={isCustomers ? ["Customer Name", "TIN", "Contact", "Invoices", "Compliance", "Actions"] : ["Supplier Name", "TIN", "Category", "Total Purchases", "Status", "Actions"]} rows={rows.map((row) => isCustomers ? ({ "Customer Name": <b>{row[0]}</b>, TIN: row[1], Contact: <span className="whitespace-pre-line">{row[2]}</span>, Invoices: row[3], Compliance: <StatusBadge>{row[4]}</StatusBadge>, Actions: rowActions(undefined, row[0]) }) : ({ "Supplier Name": <b>{row[0]}</b>, TIN: row[1], Category: row[2], "Total Purchases": row[3], Status: <StatusBadge>{row[4]}</StatusBadge>, Actions: rowActions(undefined, row[0]) }))} footer={`Showing 1 to ${Math.min(rows.length, 4)} of ${isCustomers ? "1,284 customers" : "128 suppliers"}`} />
    </>
  );
}
