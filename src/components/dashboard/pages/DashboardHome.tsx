  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div><h1 className="text-3xl font-extrabold text-[#191C1E]">Company Dashboard</h1><p className="mt-1 text-sm text-[#454557]">Monitor invoices, compliance readiness, payments, and records health.</p></div>
          ...
          <Card className="p-6">
            ...
            <div className="mt-5 space-y-3">
              {[["Customer TIN Missing (4)", "Required for B2B tax compliance", "danger"], ["VAT Configurations", "7.5% standard rate applied", "success"], ["Product HSN/Tax Codes (12)", "Missing codes for some catalog items", "danger"]].map(([title, text, tone]) => (
                <div key={title} className={`flex items-center justify-between gap-4 rounded-xl border p-4 ${tone === "danger" ? "border-red-100 bg-red-50" : "border-transparent bg-[#F1F4F8]"}`}>
          ...
          <Card className="p-6">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {[["142 Customers", "128 active this month", "/dashboard/customers", Users], ["28 Suppliers", "4 pending verification", "/dashboard/suppliers", Truck]].map(([title, text, href, Icon]) => (
                <Link key={String(title)} href={String(href)} className="flex items-center justify-between rounded-xl border border-[#C5C4DA] bg-[#F7F9FB] p-4">
