import { CreditCard, Eye, Plus, ReceiptText, ShieldCheck } from "lucide-react";
import { purchaseInvoiceRows } from "../data";
import { BottomInsight, Button, ComplianceAlert, DataTable, FilterBar, MetricCard, notifyDashboard, PageHeader, StatusBadge, rowActions } from "../ui";
...
export function PurchaseInvoicesPage() {
  return (
    <>
      ...
      <DataTable title="Invoice History" columns={["Invoice #", "Supplier", "Amount", "VAT Claimable", "Status", "Date", "Actions"]} rows={purchaseInvoiceRows.map(([invoice, supplier, amount, vat, status, date]) => ({ "Invoice #": <b className="text-[#0001B1]">{invoice}</b>, Supplier: <b>{supplier}</b>, Amount: amount, "VAT Claimable": <b className={String(vat).startsWith("₦") ? "text-green-700" : ""}>{vat}</b>, Status: <StatusBadge>{status}</StatusBadge>, Date: date, Actions: rowActions(undefined, invoice, [{ label: "View invoice", icon: Eye, onSelect: () => notifyDashboard(`${invoice} details opened`) }]) }))} footer="Showing 1 to 4 of 124 invoices" />
