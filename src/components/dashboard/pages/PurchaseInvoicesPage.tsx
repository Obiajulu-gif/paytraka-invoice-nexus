...
export function PurchaseInvoicesPage() {
  return (
    <>
      ...
      <DataTable title="Invoice History" columns={["Invoice #", "Supplier", "Amount", "VAT Claimable", "Status", "Date", "Actions"]} rows={purchaseInvoiceRows.map(([invoice, supplier, amount, vat, status, date]) => ({ "Invoice #": <b className="text-[#0001B1]">{invoice}</b>, Supplier: <b>{supplier}</b>, Amount: amount, "VAT Claimable": <b className={String(vat).startsWith("₦") ? "text-green-700" : ""}>{vat}</b>, Status: <StatusBadge>{status}</StatusBadge>, Date: date, Actions: rowActions(undefined, invoice) }))} footer="Showing 1 to 4 of 124 invoices" />
