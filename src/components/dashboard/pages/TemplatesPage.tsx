...
import { Button, Card, CheckLine, Input, notifyDashboard, PageHeader, StatusBadge } from "../ui";
...
export function TemplatesPage() {
  ...
  return (
    <>
      <PageHeader title="Invoice Templates" subtitle="Manage invoice layouts, branding, notes, and compliance fields." action={<><Button variant="secondary" onClick={() => notifyDashboard("Template changes discarded")}>Discard Changes</Button><Button onClick={() => notifyDashboard(`${selectedTemplate} template saved`)}><Save className="h-4 w-4" /> Save Template</Button></>} />
      <div className="grid min-w-0 gap-6 xl:grid-cols-[220px_minmax(0,1fr)] 2xl:grid-cols-[240px_minmax(0,1fr)_320px]">
        <Card className="p-5"><h2 className="font-bold uppercase text-[#454557]">Template Gallery</h2><div className="mt-4 space-y-4">{["Classic", "Modern", "Minimal", "Bold Cards"].map((template, index) => <button key={template} type="button" onClick={() => { setSelectedTemplate(template); notifyDashboard(`${template} template selected`); }} className={`w-full rounded-xl border p-4 text-left font-bold ${selectedTemplate === template ? "border-[#1117E8] text-[#0001B1]" : "border-[#C5C4DA]"}`}><div className="mb-3 h-24 rounded-lg bg-[#F1F4F8]" />{template}{index === 0 ? <StatusBadge tone="success">Default</StatusBadge> : null}</button>)}</div></Card>
